 ...
 * Description:
 * Note: 
  - SX1276WriteBuffer and SX1276ReadBuffer are two function which only relate to 
 MCU that user use
  - OnRadioRxDone and OnRadioTxDone are two function which relate to loramac class
 *
 * Author: Quang Huy
 *
...
/******************************************************************************/
/*
 * Function alterlative for callback
*/
__weak void OnRadioRxDone( u8_t *payload, u16_t size, u16_t rssi, u8_t snr )
{
		UNUSED(payload);
		UNUSED(size);
		UNUSED(rssi);
 ...
 * Function alterlative for callback
*/
__weak void OnRadioTxDone( void )
{
...
void SX1276Reset( void )
{
    GPIO_PinLow(SX_RESET);
    delay_ms( 1 );
    GPIO_PinHigh(SX_RESET);
    delay_ms( 6 );
}
...
/*
 * Two function related to which MCU were used by user
*/
